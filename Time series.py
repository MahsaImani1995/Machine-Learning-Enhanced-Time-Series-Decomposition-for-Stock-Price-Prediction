...
DF4 = tse.Get_Price_History(stock='سپیدار',
...
DF4.to_csv('D:/Bigdata/New folder/petroulstock.csv', index=False)
...
trend_component = decompose_model.trend.dropna()
...
seasonal_component = decompose_model.seasonal.dropna().reset_index()
...
residual_component = decompose_model.resid.fillna(method='bfill').values.reshape(-1, 1)
...
forecast_size = int((len(DF4) * 0.2)-6)
